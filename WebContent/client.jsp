  ...
  <thead>
   ...
   <tbody>
  	<tr>
  		<%
			try {
				String sql="select top "+pagesize+" * from client where id not in(select top "+pagesize*(pagenum-1)+" id from client)";		
				...
				*/
				while (rs.next()) {
					...
					out.println("<tr><td>" + rs.getInt(1) + "</td><td>" + rs.getString(2) + "</td><td>"
							+ rs.getString(3)+"</td><td>"+rs.getString(4)+"</td><td>"+rs.getString(5) +"</td><td>"+rs.getString(6)+ "</td><td><a href='updateclient.jsp?id=" + id
							+ "'>修改</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href=javascript:if(confirm('确定要删除吗?'))location='delclient.jsp?id=id'>删除</a></td></tr>");
				}
