  ...
  <thead>
   ...
   <tbody>
  	<tr>
  		<%
			try {
				String sql="select top "+pagesize+" * from orders where id not in(select top "+pagesize*(pagenum-1)+" id from orders)";
				...
				*/
				while (rs.next()) {
					...
					out.println("<tr><td>" + rs.getInt(1) + "</td><td>" + rs.getString(2) + "</td><td>"
							+ rs.getString(3)+"</td><td>"+rs.getString(4)+"</td><td>"+rs.getString(5) + "</td><td><a href='updateorder.jsp?id=" + id
							+ "'>修改</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href=javascript:if(confirm('确定要删除吗?'))location='delorder.jsp?id=id'>删除</a></td></tr>");
				}
